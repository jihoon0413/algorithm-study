package org.example.백준.알고리즘중급_1.BFS_연습.벽부수고이동하기2_14442;
...
public class Main {
        ...
        while (!que.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if(!map[x][y] && !check[x][y][breakCount]) {
                    ...
                    que.offer(new Loc(x, y, breakCount, count + 1));
                } else if(map[x][y] && breakCount < K && !check[x][y][breakCount]) {
                    check[x][y][breakCount] = true;
                    que.offer(new Loc(x, y, breakCount + 1, count + 1));
                }
    ...
    public static class Loc {
        ...
        public Loc(int x, int y , int breakCount, int count) {
            this.x = x;
            this.y = y;
